...
{ 
	...
	cout << "Введите данные о трубе: длину, диаметр" << endl;
	cin >> L >> d;
	...
	cin >> Hq >> Hs;
	cout << "Введите значения коэффициента: КПД" << endl;
	cin >> k;
	...
	U = 1.27 * (Q / pow(d, 2));
	float Re = (U * d) / 0.01;
	if (Re >= 2300) { cout << "Используется турбулентный режим\n"; D = 0.3 / (pow(Re,1/4)); } 
		else { cout << "Используется ламинарный режим\n"; D = 56 / Re; }
